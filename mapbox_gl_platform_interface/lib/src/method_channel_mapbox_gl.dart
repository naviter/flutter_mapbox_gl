...
class MethodChannelMapboxGl extends MapboxGlPlatform {
  ...
  Future<CameraPosition?> queryCameraPosition() async {
    ...
    return _cameraPosition = CameraPosition.fromMap(json);
  }

  @override
  Future<bool> setLayerVisibility(String layerId, bool isVisible) async {
    final Map<Object?, Object?> reply = await _channel.invokeMethod('map#setLayerVisibility', <String, dynamic>{
      'id': layerId,
      'visible': isVisible,
    });
  ...
  Future<Map<String, String>> getLastUsedCustomHeaders() async {
    try {
      final result = await _channel.invokeMethod<Map<dynamic, dynamic>>('map#getLastUsedCustomHeaders', <String, dynamic>{});
      return result?.map((key, value) => MapEntry<String, String>(key.toString(), value.toString())) ?? <String, String>{};
    } on PlatformException catch (e) {
      return new Future.error(e);
    }
