    ...
    func onMethodCall(methodCall: FlutterMethodCall, result: @escaping FlutterResult) {
        ...
        case "source#setFeature":
         ...
         case "map#getCustomHeaders":
            result(URLSessionConfiguration.getCustomHeaders())
         case "map#setCustomHeaders":
            ...
            URLSessionConfiguration.setCustomHeaders(headers)
            URLSessionConfiguration.setFilterForCustomHeaders(filter)
            result(nil)
        case "map#getLastUsedCustomHeaders":
            result(URLSessionConfiguration.getLastUsedCustomHeaders())
         case "map#setMaximumFps":
            result(nil)
         case "map#forceOnlineMode":
            // not implemented set, needs custom iOS build
            result(nil)
         ...
         case "map#setLayerVisibility":
            ...
            guard let layerVisible = arguments["visible"] as? Bool else { return }
         ...
         case "map#editGeoJsonSource":
            ...
            if let data = srcData.data(using: String.Encoding.utf8) {
                ...
                if src != nil && src is MGLShapeSource {
                    let geojsonSrc = src as! MGLShapeSource
                    let geojsonData = try? MGLShape(data: data, encoding: String.Encoding.utf8.rawValue)
                    if geojsonData != nil {
        ...
        case "map#toScreenCoortinates":
            ...
            let tapCoordinate: CLLocationCoordinate2D = CLLocationCoordinate2DMake(latitude, longitude)
            let tapPoint: CGPoint = mapView.convert(tapCoordinate, toPointTo: nil)
            let ret = ["latitude": tapPoint.x * scale, "longitude": tapPoint.y * scale]
            result(ret)
        default:
