...
class MapboxMapController extends ChangeNotifier {
  ...
  Future<Map<String, String>> getCustomHeaders() async {
    return await _mapboxGlPlatform.getCustomHeaders();
  }

  Future<void> setCustomHeaders(Map<String, String> headers, List<String> filter) async {
    await _mapboxGlPlatform.setCustomHeaders(headers, filter);
  }

  Future<Map<String, String>> getLastUsedCustomHeaders() async {
    return await _mapboxGlPlatform.getLastUsedCustomHeaders();
  }
  ...
  Future<void> forceOnlineMode() async {
    await _mapboxGlPlatform.forceOnlineMode();
  }

  Future<void> animateCameraWithDuration(CameraUpdate cameraUpdate, int duration) async {
    await _mapboxGlPlatform.animateCameraWithDuration(cameraUpdate, duration);
  }

  Future<CameraPosition?> queryCameraPosition() async {
    return await _mapboxGlPlatform.queryCameraPosition();
  }

  Future<bool> setLayerVisibility(String layerId, bool isVisible) async {
  ...
  Future<List> querySourceFeatures(String sourceId, List<String> sourceLayers, {String? filter}) async {
    return await _mapboxGlPlatform.querySourceFeatures(sourceId, sourceLayers, filter: filter);
  }
